...
class UpYun extends Component {
    ...
    public function init() {/*{{{*/
        $this->endpoint = is_null($this->endpoint) ? self::ED_AUTO : $this->endpoint;
    }/*}}}*/
    ...
    public function saveAs($src_file, $dest_file, $deleteTempFile=true)
    {
        $opts = array();
        if ($this->type=='image') {
            $opts = array(
        ...
        $rsp = $this->writeFile($dest_file, $fh, True, $opts);   // 上传图片，自动创建目录
        fclose($fh);
